...
//    float val0 = 0;
    ...
    if (brightcurve) {
//        val0 = brightcurve->getVal(val0);    // TODO: getVal(double) is very slow! Optimize with a LUTf //todo hadar!!
    }
    ...
    for (int i = 1; i < 0x10000; i++) {
        ...
        // apply brightness curve
        if (brightcurve) {
//            val = LIM01<float>(brightcurve->getVal (val));    // TODO: getVal(double) is very slow! Optimize with a LUTf //todo hadar!!
        }
    ...
    if (!curvePoints2.empty() && curvePoints2[0] > DCT_Linear && curvePoints2[0] < DCT_Unchanged) {
        ...
        if (!tcurve.isIdentity()) {
       //     customToneCurve2.Set(tcurve, gamma_); //tod hadar
        }
    ...
    if (!curvePoints.empty() && curvePoints[0] > DCT_Linear && curvePoints[0] < DCT_Unchanged) {
        ...
        if (!tcurve.isIdentity()) {
        //    customToneCurve1.Set(tcurve, gamma_); //todo hadar
        }
...
// void PerceptualToneCurve::initApplyState(PerceptualToneCurveState & state, const std::string &workingSpace) const
// {
//
