...
public:
    ...
    static std::unique_ptr<DiagonalCurve> createDiagonalCurve(const std::vector<double>& points, int poly_points = 1000);
    static bool diagonalCurve2Lut(const std::vector<double>& curvePoints, std::vector<float>& curve, int skip = 1);
};
...
public:
    ...
    ~DiagonalCurve() override = default;
...
public:
    static void init();
    void initApplyState(PerceptualToneCurveState & state, const std::string& workingSpace) const;
