...
public:
    ...
    std::uint8_t     getInputIntents(const std::string& name) const;
    // std::uint8_t     getInputIntents(const Glib::ustring& name) const;
    std::uint8_t     getOutputIntents(const std::string& name) const;
    // std::uint8_t     getOutputIntents(const Glib::ustring& name) const;
    // std::uint8_t     getProofIntents(const Glib::ustring& name) const;
    std::uint8_t     getProofIntents(const std::string& name) const;

    /*static*/ std::vector<Glib::ustring> getWorkingProfiles();

    static cmsHPROFILE createFromMatrix(const double matrix[3][3], bool gamma = false, const std::string& name = std::string());
